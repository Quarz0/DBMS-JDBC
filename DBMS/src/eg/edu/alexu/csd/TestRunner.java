...
public class TestRunner {
    ...
    public static String[] run(String resourceDir, String testPackage, List<String> classes)
            throws Throwable {
        ...
        try {
            ...
            log.append(
                    ...
                    .append("\n");
            log.append("Please review the 'Sources' list above for the detected source-code")
                    .append("\n");
            log.append("\t").append(showError(e)).append("\n}}}");
            return new String[] { "Compile", log.toString(), "" };
        }
        log.append("UnitTests: " + (tests.isEmpty() ? "none!" : Arrays.toString(tests.toArray())))
                .append("\n");

        log.append("Classes: " + (classes.isEmpty() ? "none!" : Arrays.toString(classes.toArray())))
                .append("\n");
        int failed = 0;
        int total = 0;
        ...
        for (Class<?> testClass : tests) {
            ...
            try {
                ...
                log.append("\tImplementations: "
                        + (impls.isEmpty() ? "{{{none!}}}" : Arrays.toString(impls.toArray())))
                        .append("\n");
            } catch (Throwable e) {
